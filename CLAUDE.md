...
file in `src/components/`. `chess.js` 1.4.0 is the only non-platform dependency,
an ordinary npm import (the sandbox transpile preserves the BigInt literals its
Zobrist hashing uses; the pin stays exact because the engine reaches into its
private move generator).
